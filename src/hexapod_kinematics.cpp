...
namespace hexkins
{
  ...
  static std::array<Eigen::Vector3d, 6> dummy_joint_info;
      ...
      const std::array<Eigen::Vector3d, 6> &platform_joint_directions)
  {
    ...
    Eigen::Matrix<double, 6, 1> StrutLengthDiff;
      ...
      const std::array<Eigen::Vector3d, 6> &platform_joint_directions)
  {
    ...
    double avg_arm_length = std::accumulate(
                                joints.begin(),
                                joints.end(), 0.0, [](double a, double b)
                                { return a + b; }) /
                            6;
      ...
      const Eigen::Quaterniond &current_orientation)
  {
    Eigen::Vector3d aw;
    Eigen::Matrix3d RMatrix;
    std::array<Eigen::Vector3d, 6> struts;
